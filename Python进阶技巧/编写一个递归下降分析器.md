...
我们来尝试编写一个编译原理课程的基础作业：计算器。

### 首先我们需要一个Lexer，进行词法拆分
    ...
    def lex(self, code):
        ...
        for m in iter(scanner.match, None):
            ...
            if ttype == Token.Type.ILLEGAL:
                raise SyntaxError(f"Illegal character '{ttext}'\n")
...
class Parser:
    ...
    def expr(self):
        op = self.eval()
        params = []
        while not self.accept(Token.Type.RPAREN):
            n = self.eval()
            params.append(n)
        val = reduce(op, params[1:], params[0])
        return val

    def eval(self):
        if self.accept(Token.Type.NUM) or \
           self.accept(Token.Type.ADD) or \
           self.accept(Token.Type.SUB) or \
           self.accept(Token.Type.MUL) or \
           self.accept(Token.Type.DIV):
            tok = self.current_token()
            return tok.tvalue
        if self.accept(Token.Type.LPAREN):
            val = self.expr()
            return val
        raise SyntaxError("Expected number")
...
lexer = Lexer()
parser = Parser(lexer)
print(parser.parse("3.0"))
print(parser.parse("(+ 1 2 3)"))
print(parser.parse("(* 2.0 (+ 1 2 3) (/ 9.0 3.0))"))
```
