...
遇到在类的方法中使用类本身作为类型注解的时候，或者类与类的循环依赖，会引发类型不存在的错误，不仅静态检查无法通过，解释器执行的时候都会报错，因为到类的代码执行完成之前，类还没真正诞生。可以使用 `"ClassName"` 来表示即将出现的类。
...
遇到类型A的方法依赖类型B，类型B的方法依赖类型A，也是一样的处理方式。
